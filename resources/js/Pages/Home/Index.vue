<template>
<div class="font-montserrat text-sm bg-black" >
    <div class="flex min-h-screen  2xl:max-w-screen-2xl 2xl:mx-auto 2xl:border-x-2 2xl:border-gray-200 dark:2xl:border-zinc-700 ">
        ...
        <aside class=" w-1/6 py-10 pl-10  min-w-min  border-r border-gray-300 dark:border-zinc-700  hidden md:block ">
            <a href="/">
                <div class=" font-bold text-lg flex items-center gap-x-3">
                    <svg width="50" height="50">
                        <circle cx="50" cy="50" r="100" stroke="green" stroke-width="4" fill="orange" />
                    </svg>        
                    <div class="tracking-wide dark:text-white">Ko Aung Thu <span class="text-red-600">Code Test</span></div>
                </div>
            </a>
            ...
            <div class="mt-12 flex flex-col gap-y-4 text-gray-500 fill-gray-500 text-sm">
                ...
                <a v-if="is_auth" class="flex items-center space-x-2 py-1 text-red group hover:border-r-4 hover:border-r-red-600 hover:font-semibold dark:hover:text-white " href="/admin">
                    ...
                    <span>Upload Movie/ Create movie</span>
                </a>
                <a v-else class="flex items-center space-x-2 py-1 text-red group hover:border-r-4 hover:border-r-red-600 hover:font-semibold dark:hover:text-white " href="/login">
                    <svg class="h-5 w-5 group-hover:fill-red-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        <g>
                            <path d="M12 2C6.48 2 2 6.48 2 12c0 5.51 4.48 10 10 10 5.51 0 10-4.49 10-10 0-5.52-4.49-10-10-10Zm0 18c-4.42 0-8-3.59-8-8 0-4.42 3.58-8 8-8 4.41 0 8 3.58 8 8 0 4.41-3.59 8-8 8Z"></path>
                            <path d="M13 7h-2v5.414l3.29 3.29 1.41-1.42 -2.71-2.71Z"></path>
                ...
                <a class=" flex items-center space-x-2 py-1  group hover:border-r-4 hover:border-r-red-600 hover:font-semibold dark:hover:text-white " href="/movies">                  
                    <svg class="h-5 w-5 group-hover:fill-red-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        <g>
        ...
        <main class=" flex-1 py-10  px-5 sm:px-10 ">
            ...
            <header class=" font-bold text-lg flex items-center  gap-x-3 md:hidden mb-12 ">
                ...
                <div class="tracking-wide dark:text-white flex-1">Code Test<span class="text-red-600">.</span></div>
            ...
            <section>
                <nav class="flex space-x-6 text-gray-400 font-medium item-center  ">
                    <a v-for="item in genres" v-bind:key="item.id"
                         href="#" class="hover:text-gray-700 dark:hover:text-white">{{ item.name }}</a>


                         <a v-if="is_auth" href="/admin" class="px-5 py-2.5 bg-red-600 hover:bg-red-700 rounded-lg text-center font-medium block text-white">Create movie/ Upload movie</a>
                         <a v-else href="/login" class="px-5 py-2.5 bg-red-600 hover:bg-red-700 rounded-lg text-center font-medium block text-white">Create movie/ Upload movie</a>
                </nav>
                ...
                <div class="flex flex-col justify-between mt-4 bg-black/10 bg-blend-multiply rounded-3xl h-80 overflow-hidden bg-cover bg-center px-7 pt-4 pb-6 text-white"
                 style="background-image: url('images/banner_01.jpeg');" >
                    <!-- <img class="object-cover w-full h-full" src="images/inception.jpg" alt=""> -->
                    <div class="flex -space-x-1 items-center ">
                       
                        <span class="pl-4 text-xs drop-shadow-lg">Do you watched this movie?</span>
                    </div>
                    
                    <div class="bg-gradient-to-r from-black/30 to-transparent -mx-7 -mb-6 px-7 pb-6 pt-2">
                        <span class="uppercase text-3xl font-semibold drop-shadow-lg ">Dark Knight Rises</span>
            ...
            <section class="mt-9">
                <div class="flex items-center justify-between">
                    <span class="font-semibold text-gray-300 text-base dark:text-white">Latest Movies</span>
                ...
                <div class="mt-4 grid grid-cols-2 gap-y-5 sm:grid-cols-3 gap-x-5 ">
                    <MovieCard  v-for="movie in popular_movies" v-bind:key="movie.id" :movie="movie" ></MovieCard>
                </div>

                <div class="mt-4 grid grid-cols-2 gap-y-5 sm:grid-cols-3 gap-x-5 ">
                    <div></div>
                    <div class="items-center justify-between">
                        <a href="/watch" class="px-5 py-2.5 bg-red-600 hover:bg-red-700 rounded-lg text-center font-medium block text-white">See More</a>
                    </div>
                    <div></div>
                </div>
        ...
        <aside class=" w-1/5 py-10 px-10  min-w-min  border-l border-gray-300 dark:border-zinc-700 hidden lg:block ">
            ...
            <div class="mt-10">
                <span class="font-semibold text-white-700 dark:text-white">Popular Movies</span>
                <ul class="mt-4 text-white-400 text-xs space-y-3">
                    <li v-for="movie in popular_movies" v-bind:key="movie.id" class="flex space-y-3 space-x-3 ">
                        <img :src="movie.cover_image" class="w-1/3 rounded-md" alt="">
                        <div class="flex flex-col justify-between  ">
                            <div class="flex flex-col space-y-1">
                                <span class="text-white dark:text-white font-semibold">{{ movie.title }}</span>
                                <span class="text-xxs hidden xl:block">{{ movie.imdb_ratings}}</span>
                            </div>
            ...
            <div class="mt-10">
                ...
                <ul class="mt-4 text-gray-400 text-xs space-y-3">
                    <li v-for="movie in popular_movies" v-bind:key="movie.id" class="flex space-y-3 space-x-3 ">
                        <img :src="movie.cover_image" class="object-cover w-1/3 rounded-md" alt="">
                        <div class="flex flex-col justify-between  ">
                            <div class="flex flex-col space-y-1">
                                <span class="text-gray-700 dark:text-white font-semibold">{{ movie.title }}</span>
